start_server {tags {"zset"}} {
    ...
    proc basics {encoding} {
        ...
        test "ZSET element can't be set to NaN with ZADD - $encoding" {
            assert_error "ERR*" {r zadd myzset nan abc}
        }
        ...
        test "ZREMRANGEBYRANK with non-value start or stop" {
            assert_error "ERR*" {r zremrangebyrank fooz str 1}
            assert_error "ERR*" {r zremrangebyrank fooz 1 str}
            assert_error "ERR*" {r zremrangebyrank fooz 1 NaN}
        }
    ...
    proc stressers {encoding} {
        ...
        test "ZSCORE zset not exist - $encoding" {
            r del zscoretest
            assert_equal {} [r zscore zscoretest foo]
        }
        ...
        #     r del zset
        #     for {set i 0} {$i < $elements} {incr i} {
        #         r zadd zset [expr int(rand())] $i
        #     }

        #     # assert_encoding $encoding zset
        #     for {set i 0} {$i < 100} {incr i} {
        #         set min [expr int(rand())]
        #         set max [expr int(rand())]
        #         if {$min > $max} {
        #             set aux $min
        #             set min $max
        #             set max $aux
        ...
        test "ZSETs skiplist implementation backlink consistency test - $encoding" {
            set diff 0
            for {set j 0} {$j < $elements} {incr j} {
                r zadd myzset [expr int(rand())] "Element-$j"
    ...
    proc maxlen {} {
        test "ZADD maxlen 5 pruning default (min)" {
            r del zset
            r zadd zset maxlen 5 1 a 2 b 3 c
            set a [r zrange zset 0 -1]
            r zadd zset 4 d 5 e 6 f
            set b [r zrevrange zset 0 -1]
            list $a $b
        } {{a b c} {f e d c b}}

        test "ZADD maxlen 5 pruning max" {
            r del zset
            r zadd zset maxlen 5 pruning max 1 a 2 b 3 c
            set a [r zrange zset 0 -1]
            r zadd zset 4 d 5 e 6 f
            set b [r zrevrange zset 0 -1]
            list $a $b
